...
if __name__ == '__main__':
    for test_i, mu_old in zip([1,2,3], [np.asarray([0.25, 0.25, 0.25, 0.25]), np.asarray([1.0, 0.0, 0.0, 0.0]), np.asarray([0.0, 0.0, 0.0, 1.0])]):
        ...
        Nt_pde = npzfile['Nt']
        Dt_pde = T_pde / Nt_pde
        Nt_pde = int(T_tot / Dt_pde)
        plt.plot(np.linspace(0,Nt_pde,Nt_pde+1)*Dt_pde, mu_pde[:Nt_pde+1, 0], label="mu_pde_MFC[0]", color='xkcd:steel', linewidth=2)
        plt.plot(np.linspace(0,Nt_pde,Nt_pde+1)*Dt_pde, mu_pde[:Nt_pde+1, 1], label="mu_pde_MFC[1]", color='xkcd:bright magenta', linestyle='--', linewidth=2)
        plt.plot(np.linspace(0,Nt_pde,Nt_pde+1)*Dt_pde, mu_pde[:Nt_pde+1, 2], label="mu_pde_MFC[2]", color='xkcd:bright green', linestyle=':', linewidth=2)
        plt.plot(np.linspace(0,Nt_pde,Nt_pde+1)*Dt_pde, mu_pde[:Nt_pde+1, 3], label="mu_pde_MFC[3]", color='xkcd:royal blue', linestyle='-.', linewidth=2)
        ...
        time_steps_pde = np.asarray(range(Nt_pde+1))
        plt.plot(time_steps_pde*Dt_pde, t_c_pde[:Nt_pde+1], label="V_MFC", color='xkcd:purple', linewidth=2, ls='--')
        Q_mu_pde_tmp = [np.max(Q[proj_W_index(mu_pde[it])]) for it in time_steps_pde]
        Q_mu_pde = -np.asarray(Q_mu_pde_tmp)
        plt.plot(time_steps_pde*Dt_pde, Q_mu_pde, label="Q[mu_pde]", color='xkcd:orange', linewidth=2)
