...
class NashQPlayer():
    ...
    def training(self):
        ...
        for i in tqdm(range(self.max_itrs)):
            if self.MonteCarlo:
                if np.array_equal(current_states[0],predator_terminate_state) or np.array_equal(current_states[1] ,preyer_terminate_state):
                    current_states = [self.Q_predator.states[-1],self.Q_preyer.states[0]]
                    continue

                if self.decision_strategy == "random":
                    i_alpha_1 = self.table.controls[rnd.randrange(self.Q_predator.n_controls)]
                    i_alpha_2 = self.table.controls[rnd.randrange(self.Q_preyer.n_controls)]

                if self.decision_strategy == "epsilon-greedy":
                    random_number = rnd.uniform(0,1)
                    if random_number >=self.epsilon:
                        strategies = self.env.get_nash_Q_value(self.Q_predator.Q_table[self.Q_predator.get_state_index(current_states[0])],
                        self.Q_preyer.Q_table[self.Q_preyer.get_state_index(current_states[1])])

                        i_alpha_1 = np.argmax(strategies[0])
                        i_alpha_2 = np.argmax(strategies[1])

                    else:
                        i_alpha_1 = rnd.randrange(self.Q_predator.n_controls)
                        i_alpha_2 = rnd.randrange(self.Q_preyer.n_controls)
                ...
                self.Q_predator.Q_table[self.Q_predator.get_state_index(current_states[0])][i_alpha_1][i_alpha_2] = ((1-self.lr)*self.Q_predator.Q_table[self.Q_predator.get_state_index(current_states[0])][i_alpha_1][i_alpha_2]
                    + self.lr*(r_next_1 + self.disct_fct * np.dot(np.dot(pi_1, self.Q_predator.Q_table[i_mu_1_next]),pi_2)))

                self.Q_preyer.Q_table[self.Q_preyer.get_state_index(current_states[0])][i_alpha_1][i_alpha_2] = ((1-self.lr)*self.Q_preyer.Q_table[self.Q_preyer.get_state_index(current_states[0])][i_alpha_1][i_alpha_2]
                    + self.lr*(r_next_2 + self.disct_fct * np.dot(np.dot(pi_1, self.Q_preyer.Q_table[i_mu_2_next]),pi_2)))
            else:
                for i_mu in range(self.Q_predator.n_states):
                    # print("i_mu = {}\n".format(i_mu))

                    mu_1 = self.Q_predator.states[self.Q_predator.n_states-1-i_mu] # initial predator starts from left
                    mu_2 = self.Q_preyer.states[i_mu] # preyer starts from the right
                    for i_alpha_1 in range(self.Q_predator.n_controls):

                        for i_alpha_2 in range(self.Q_preyer.n_controls):

                            # print("i_alpha = {}\n".format(i_alpha))
                            alpha_1 = self.Q_predator.controls[i_alpha_1]
                            alpha_2 = self.Q_preyer.controls[i_alpha_2]
                            next_mu_1 = self.env.get_next_mu(mu_1,alpha_1)
                            next_mu_2 = self.env.get_next_mu(mu_2,alpha_2)

                            
                            i_mu_1_next = self.Q_predator.proj_W_index(next_mu_1) # find its most nearest mu
                            i_mu_2_next = self.Q_preyer.proj_W_index(next_mu_2) # find its most nearest mu
    ...
    def recover_equilibrium_policy(self,starting_states,Q_1, Q_2,env):
        ...
        policy_predator = []
        policy_preyer = []
        while not np.array_equal(current_states[0] , preyer_terminate_state) or not np.array_equal(current_states[1] ,predator_terminate_state):
